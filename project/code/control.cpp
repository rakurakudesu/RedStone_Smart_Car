...
const float encoder_filter = 0.02;//编码器滤波系数(越小越强)
...
void line_follow_pid_control(void)
{
    ...
    // ===== target_omega 变化率限制（声明提前，丢线逻辑需要引用）=====
    static float smooth_target_omega = 0;
    #define OMEGA_SLEW 1.0f  // 每周期最大变化 3.0 rad/s，大弯减少相位滞后

    // ===================== 1. 外环：图像偏差 → 目标角速度 =====================
    extern bool line_valid;  // 来自 image_basic.cpp，标记当前帧是否成功找到线
    static uint8 line_lost_cnt = 0;  // 连续丢线计数
    float center_error = get_center_error();

    if (line_valid) {
        line_lost_cnt = 0;
        target_omega = PID_Positional_Calculate(&TracePID, center_error, 0.0f);
    } else {
        line_lost_cnt++;
        if (line_lost_cnt > 40) {
            // 丢线超过 200ms，缓慢回正（slew limiter 会把 smooth 带向 0）
            target_omega = 0.0f;
        } else {
            // 短期丢线：冻结转向，保持最后有效角速度，避免在旧数据上乱转
            target_omega = smooth_target_omega;
        }
    }
    ...
    // ===================== 4. 差速环：编码器 → 差速 =====================
    target_delta_Sp = target_lspeed - target_rspeed;  // 与速度目标差对齐，差速环和速度环同向发力
